...
public class Table extends Observable {
  ...
  private Table() {
    defaultPieceIconPath = "src/main/resources/art/pieces/chess_com/neo/";
    ending = ".gif";
  ...
  private JMenu createPreferencesMenu() {
    ...
    final JCheckBoxMenuItem legalMoveHighLighterCheckBox = new JCheckBoxMenuItem(
  ...
  private JMenu createOptionsMenu() {
    ...
    evaluateBoardMenuItem.addActionListener(e -> System.out.println(
  ...
  private JMenu createDesignMenu() {
    ...
    neoMenuItem.addActionListener(e -> {
      defaultPieceIconPath = "src/main/resources/art/pieces/chess_com/neo/";
      ending = ".gif";
      boardPanel.drawBoard(chessBoard);
    });
    ...
    simpleMenuItem.addActionListener(e -> {
      defaultPieceIconPath = "src/main/resources/art/pieces/black_widow/simple/";
      ending = ".gif";
      boardPanel.drawBoard(chessBoard);
    });

    final JRadioButtonMenuItem experimentalMenuItem = new JRadioButtonMenuItem(
        "Experimental (3D-Lichess)");
    experimentalMenuItem.addActionListener(e -> {
      defaultPieceIconPath = "src/main/resources/art/pieces/lichess/3D/Experimental/";
      ending = ".png";
      boardPanel.drawBoard(chessBoard);
    });
  ...
  private void undoAllMoves() {
    for (int i = Table.get().getMoveLog().size() - 1; i >= 0; i--) {
      final Move lastMove = Table.get().getMoveLog()
          .removeMove(Table.get().getMoveLog().size() - 1);
  ...
  private static class TableGameAIWatcher implements Observer {
    ...
    public void update(final Observable o,
      ...
      if (Table.get().getGameSetup().isAIPlayer(Table.get().getGameBoard().getCurrentPlayer()) &&
        ...
        System.out.println(
            Table.get().getGameBoard().getCurrentPlayer() + " is set to AI, thinking....");
      ...
      if (Table.get().getGameBoard().getCurrentPlayer().isInCheckMate()) {
        JOptionPane.showMessageDialog(Table.get().getBoardPanel(),
            "Game Over: Player " + Table.get().getGameBoard().getCurrentPlayer()
    ...
    public void done() {
      try {
        ...
        Table.get().updateGameBoard(
            Table.get().getGameBoard().getCurrentPlayer().makeMove(bestMove).getToBoard());
        Table.get().getMoveLog().addMove(bestMove);
        Table.get().getGameHistoryPanel()
  ...
  private class TilePanel extends JPanel {
      ...
      if (board.getTile(tileId).isTileOccupied()) {
        try {
          Image image = ImageIO.read(new File(
                  defaultPieceIconPath + board.getTile(tileId).getPiece().getPieceAlliance().toString()
                      .substring(0, 1) + board.getTile(tileId).getPiece().toString() + ending))
      ...
      if (highlightLegalMoves) {
        for (final Move move : pieceLegalMoves(board)) {
          if (move.getDestinationCoordinate() == tileId) {
            try {
              add(new JLabel(
                  new ImageIcon(ImageIO.read(new File("src/main/resources/art/misc/green_dot.png")))));
            } catch (Exception e) {
              e.printStackTrace();
            }
          }
    ...
    private Collection<Move> pieceLegalMoves(final Board board) {
      if (humanMovedPiece != null && humanMovedPiece.getPieceAlliance() == board.getCurrentPlayer()
          .getAlliance()) {
        ...
        return ImmutableList.copyOf(Iterables.concat(humanMovedPiece.calculateLegalMoves(board),
            chessBoard.getCurrentPlayer()
                .calculateKingCastles(chessBoard.getCurrentPlayer().getLegalMoves(),
